...
// // TestConcurrentSafety_BasicOperations 测试基本操作的并发安全性
// func TestConcurrentSafety_BasicOperations(t *testing.T) {
// 	t.Log("=== 并发安全性测试 - 基本操作 ===")
...
// 	// 启动多个goroutine进行并发读操作
// 	for i := 0; i < numGoroutines; i++ {
// 		wg.Add(1)
// 		go func(goroutineID int) {
// 			defer wg.Done()
// 			for j := 0; j < numOperations; j++ {
// 				key := fmt.Sprintf("key_%d_%d", goroutineID, j%100) // 读取部分存在的key

// 				// 并发Get操作
// 				_, _, err := cluster.Get(key)
// 				if err != nil {
// 					t.Errorf("Get操作失败: %v", err)
// 					return
// 				}
// 			}
// 		}(i)
...
// 	t.Logf("并发操作完成，集群统计: %+v", stats)

// 	// 验证没有数据竞争导致的panic
// 	t.Log("✅ 并发基本操作测试通过")
// }

// // TestConcurrentSafety_NodeOperations 测试节点操作的并发安全性
// func TestConcurrentSafety_NodeOperations(t *testing.T) {
// 	t.Log("=== 并发安全性测试 - 节点操作 ===")
...
// 	go func() {
// 		defer wg.Done()
// 		err := cluster.AddNode("node3:8003")
// 		if err != nil {
// 			t.Errorf("添加节点失败: %v", err)
// 		}
// 		t.Log("节点添加完成")
// 	}()

// 	// goroutine 2: 同时进行读写操作
// 	go func() {
// 		defer wg.Done()
// 		for i := 0; i < 50; i++ {
...
// 	// 验证最终状态
// 	stats := cluster.GetStats()
// 	migrationStats := cluster.GetMigrationStats()

// 	t.Logf("最终集群统计: %+v", stats)
// 	t.Logf("迁移统计: 迁移键数=%d", migrationStats.MigratedKeys)

// 	t.Log("✅ 并发节点操作测试通过")
// }

// // BenchmarkConcurrentOperations 并发性能基准测试
...
// 	// 添加新节点 - 这应该触发数据迁移
// 	err := cluster.AddNode("node4:8004")
// 	if err != nil {
...
// 	// 获取迁移统计
// 	stats := cluster.GetMigrationStats()
...
// 	// 移除node2 - 这应该触发数据迁移
// 	err := cluster.RemoveNode("node2:8002")
// 	if err != nil {
...
// 	// 创建初始集群
// 	cluster := core.NewDistributedCache([]string{"node1:8001", "node2:8002"})
...
// 	cluster := core.NewDistributedCache(nodes)
